...
public class ChoiceBank2ButtonHandler implements Handlers {
    @Override
    public SendMessage sendMessage(Long userId) {
        SendMessage message = new SendMessage();
        message.setText("Ви обрали Банк: " + Buttons.BANK2.get());
        message.setChatId(userId);

        InlineKeyboardButton bank1 = InlineKeyboardButton.builder().text(Buttons.BANK1.get()).callbackData(Buttons.BANK1.get()).build();
        InlineKeyboardButton bank2 = InlineKeyboardButton.builder().text(Icons.CHECK.get() + " " + Buttons.BANK2.get()).callbackData(Buttons.BANK2.get()).build();
